...
A `tool_*` expectation labeled as outcome must declare `outcome_basis: "terminal-action"`. Accept that basis only when the action or returned side effect is itself what the user consumes. Reject internal peer calls, routing, command launch, or orchestration marked this way; those remain mechanism evidence even when they are required for the outcome.
